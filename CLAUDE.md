...
`configs/proven_v1.yaml` contains validated hyperparameters locked after 6 experiments on 3-speaker CommonVoice audio (80-band log-mel, SR=16000). Do not change these values without re-running the full experiment suite. Key deviations from `DREAMConfig` defaults: `base_plasticity=0.4`, `forgetting_rate=0.03`, `base_threshold=0.35`, `surprise_temperature=0.12`, `ltc_tau_sys=5.0`.

## Code Style

- Python 3.13+, line length 100
- Ruff with E/W/F/I/B/C4/UP/ARG/TCH rules (E501, B028, B904 ignored)
- Mypy strict mode; `torch.*`, `torchaudio.*`, `matplotlib.*`, `numpy.*` imports are ignored
